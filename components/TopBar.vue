<template>
    <div>
    <header class="flex flex-col lg:flex-row items-center justify-between px-6 py-4 bg-white border-b-4 border-purple-600">
  ...
  export default {
    data() {
      return {
        fromDate: '', // Starting from the first of the last six months
        toDate: '',
        selectedDate: '',
        searchQuery: '',
        filteredCustomers: [] // New array to hold filtered Customers
      };
    },
    ...
    methods: {
      setDefaultFromMonth() {
        const now = new Date();
        const sixMonthsAgo = new Date(now.getFullYear(), now.getMonth() - 6, 1); // Six months calculations
        const formattedDate = this.formatDate(sixMonthsAgo);
      ...
      filterByDateRange() {
        this.filteredCustomers = this.customer.filter(customer => {
          const customerDate = new Date(customer.dateOfBirth);
          return customerDate >= new Date(this.fromDate) && customerDate <= new Date(this.toDate);
        });
      },
      ...
      searchCustomer() {
        ...
        else{this.filteredCustomers = this.customer}
