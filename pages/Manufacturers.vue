...
const deleteManufacturer = async (manufacturer, index) => {
    try {
    const response = await fetch(`https://retoolapi.dev/iU7LpP/manufacturer0098?id=${manufacturer.id}`, {
      method: 'DELETE',
    });
...
const filteredManufacturers = computed(() => {
  ...
  return manufacturer.filter((manufacturer) => {
...
<template>
    ...
    <div class="div1 card bg-white relative border rounded-lg">
      ...
      <div class="tableFixHead" style="padding: 0rem 1rem">
        <table class="w-full text-sm text-left text-gray-300">
          ...
          <tbody>
            <tr
              v-for="(manufacturer, index) in filteredManufacturers"
              :key="manufacturer.id"
            ...
            >
              ...
              <td class="text-center d-flex font-medium">
                <Icon
                  ...
                  @click="editManufacturer(manufacturer, index)"
                  title="Edit"
                >
                </Icon>
                <Icon
                  ...
                  @click="deleteManufacturer(manufacturer, index)"
                  title="Delete"
                >
                </Icon>
              </td>
  ...
  <div v-if="showModal" class="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-75">
    ...
    <div v-if="editSelectedManufacturer" class="fixed inset-0 flex justify-center items-center bg-gray-800 bg-opacity-75">
    <div class="bg-white rounded-lg shadow-md p-8 max-w-md w-full">
